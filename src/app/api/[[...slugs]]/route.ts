...
const rooms = new Elysia({ prefix: "/room" }).post("/create", async () => {
    const roomId = nanoid()
    //stores metadata for the chat room
    await redis.hset(`meta:${roomId}`, {
        connected: [],
        createdAt: Date.now(),
    });

    //autodeleteion by reddis expiry
...
const messages = new Elysia({ prefix: "/messages" }).use(authMiddleware).post("/", async ({ body, auth }) => {
    ...
    query: z.object({ roomId: z.string() }),
    body: z.object({
        sender: z.string().max(100),//user id cannot be longer than 100
        text: z.string().max(1000),//maximum text will be of 1000 charecters
    }),
})
    .get("/",
        async ({ auth }) => {
